...
public class HigherOrderUtils {
    ...
    public static NamedBiFunction<Double, Double, Double> divide = new NamedBiFunction<Double, Double, Double>() {
        ...
        public Double apply(Double aDouble, Double aDouble2) {
            try {
                return (double) (aDouble.intValue() / aDouble2.intValue());
            }
    ...
    public static <T> T zip(List<T> args, List<NamedBiFunction<T, T, T>> bifunctions) {
        try {
            if (bifunctions.size() >= args.size())
                throw new IndexOutOfBoundsException("More functions than elements to operate on or args doesn't have enough elements");
            IntStream.range(0, bifunctions.size()).forEach(i -> args.set(i + 1, bifunctions.get(i).apply(args.get(i), args.get(i + 1))));
